...
public class Gameplay extends JPanel implements KeyListener, ActionListener {
	...
	public void paint(Graphics g) {
		...
		//draw bonus
				if(lengthofsnake%10==0) {
					bonusvisible=true;
				}
				
				if(bonusvisible==true){
				    g.setColor(Color.RED);
				    g.fillRect(enemyxpos[bonxpos], enemyypos[bonypos], 25, 25);
				    g.setColor(Color.BLUE);
				    g.drawString("Time Left: ",150,30);
				    g.setColor(Color.GREEN);
				    g.drawRect(250, 10, 100, 20);
				    g.fill3DRect(250, 10, bonus-=1, 20,false);
				     if(bonus==0){
				    	 bonusvisible=false;
				    	 bonus=100;
				    }
		...
		if(enemyxpos[bonxpos]==snakexlength[0] && enemyypos[bonypos]==snakeylength[0]) {
			score+=bonus*2;
			lengthofsnake++;
	 ...
	 if((enemyxpos[xpos]==175 && enemyypos[ypos]==75+25*j)||(enemyxpos[xpos]==450)||(enemyxpos[xpos]==725 && enemyypos[ypos]==150+25*j)){
		...
		enemyimage.paintIcon(this, g, enemyxpos[xpos], enemyypos[ypos]);
			...
			if(over==true) {
				...
				g.drawString("Press ENTER to RESTART", 300, 340);
			}
		g.dispose();
	}
